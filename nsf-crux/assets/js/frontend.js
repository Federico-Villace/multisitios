(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfcruxReady === '1') return;
    scope.dataset.nsfcruxReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfcrux-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfcrux-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfcrux-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfcrux-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfcrux-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfcrux-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfcrux-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfcrux-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfcrux-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfcrux-search-open');
      if (!document.body.classList.contains('nsfcrux-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfcrux-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfcrux-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfcrux-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfcrux_header_news.default', initAll);
  }
})();
