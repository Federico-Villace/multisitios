...
abstract class Widget_Base extends \Elementor\Widget_Base {
    public function get_categories() {
        return [ 'nsfcrux-widgets' ];
    }

    public function get_style_depends() {
        return [ 'nsfcrux-widgets-frontend' ];
    }
    ...
    protected function add_query_controls( $default = 4 ) {
        $this->add_control(
            'posts_per_page',
            [
                'label'   => esc_html__( 'Cantidad de notas', 'nsfcrux-widgets' ),
        ...
        $this->add_control(
            'categories',
            [
                'label'       => esc_html__( 'Categorías', 'nsfcrux-widgets' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => 'date',
                'options' => [
                    'date'     => esc_html__( 'Fecha', 'nsfcrux-widgets' ),
                    'views'    => esc_html__( 'Vistas', 'nsfcrux-widgets' ),
                    'comments' => esc_html__( 'Comentarios', 'nsfcrux-widgets' ),
                    'rand'     => esc_html__( 'Aleatorio', 'nsfcrux-widgets' ),
                ],
            ]
        );
    }
    ...
    protected function render_card( $post_id, $args = [] ) {
        ...
        if ( ! empty( $args['show_author'] ) ) {
            $byline = Helpers::author_byline( $post_id, $args['author_prefix'] ?? __( 'Por', 'nsfcrux-widgets' ) );
