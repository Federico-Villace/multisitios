(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfauroraReady === '1') return;
    scope.dataset.nsfauroraReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfaurora-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfaurora-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfaurora-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfaurora-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfaurora-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfaurora-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfaurora-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfaurora-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfaurora-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfaurora-search-open');
      if (!document.body.classList.contains('nsfaurora-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfaurora-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfaurora-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfaurora-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfaurora_header_news.default', initAll);
  }
})();
