...
class Latest_Box extends Widget_Base {
    public function get_name() { return 'nsfmeridiano_latest_box'; }
    public function get_title() { return esc_html__( 'Lo Último Box', 'nsfmeridiano-widgets' ); }
        ...
        <div class="nsfmeridiano-scope"><div class="nsfmeridiano-latest-box">
            ...
            <div class="nsfmeridiano-latest-list">
                <?php $i = 0; while ( $q->have_posts() ) : $q->the_post(); $i++; ?>
                    <article class="nsfmeridiano-latest-item <?php echo 1 === $i && 'yes' === ( $s['first_with_thumb'] ?? 'yes' ) ? 'has-thumb' : ''; ?>">
                        <div class="nsfmeridiano-latest-text"><a class="nsfmeridiano-latest-item-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a></div>
                        <?php if ( 1 === $i && 'yes' === ( $s['first_with_thumb'] ?? 'yes' ) ) : ?>
                            <a class="nsfmeridiano-latest-thumb" href="<?php the_permalink(); ?>"><?php if ( has_post_thumbnail() ) { the_post_thumbnail( 'thumbnail' ); } else { echo '<span class="card-img sq ' . esc_attr( \NSFMERIDIANO\Helpers::card_image_class( get_the_ID() ) ) . '"></span>'; } ?></a>
                        <?php endif; ?>
                    </article>
