...
class Demo_Installer {
    private $option_key = 'nsfmeridiano_demo_ids';
    ...
    public function install() {
        ...
        $posts = [
            [ 'La Justicia rechazó citar a declarar a un dirigente por una liberación irregular', 'Política', 'Redacción Meridiano' ],
            ...
            [ 'Horóscopo de hoy: signo por signo, qué te espera este lunes', 'Horóscopo', 'Astróloga dMeridiano' ]
        ];
        ...
        foreach ( $posts as $item ) {
            ...
            $post_id = wp_insert_post(
                [
                    ...
                    'post_status'   => 'publish',
                    'post_type'     => 'post',
                    'post_excerpt'  => sanitize_text_field( 'Nota demo para probar la estética editorial con cards y portada de alto impacto, los listados por sección y los módulos dinámicos de Meridiano.' ),
                    'post_content'  => wp_kses_post( $this->demo_content( $item[0] ) ),
                    'post_category' => $category_id ? [ $category_id ] : [],
                    'meta_input'    => [
                        '_nsfmeridiano_demo'      => 1,
        ...
        $pages = [
            'Meridiano Demo Completo' => '[nsfmeridiano_demo view="cat" switcher="yes"]',
                        'Meridiano Nota'          => '[nsfmeridiano_demo view="nota" switcher="no"]',
            'Meridiano Categoría'     => '[nsfmeridiano_demo view="cat" switcher="no"]',
        ];
        ...
        foreach ( $pages as $title => $shortcode ) {
            ...
            if ( ! is_wp_error( $page_id ) ) {
                update_post_meta( $page_id, '_nsfmeridiano_demo_page', 1 );
        ...
        $blocks_page_id = wp_insert_post(
            [
                'post_title'   => 'Meridiano Home · Bloques nuevos',
                'post_type'    => 'page',
                'post_status'  => 'publish',
                'post_content' => '',
            ],
        ...
        if ( ! is_wp_error( $blocks_page_id ) ) {
            update_post_meta( $blocks_page_id, '_nsfmeridiano_demo_page', 1 );
        ...
        $menu_id = wp_create_nav_menu( 'Meridiano Demo' );
    ...
    private function elementor_blocks_data() {
        ...
        return [
            $section( [ $widget( 'nsfmeridiano_header_news', [] ) ] ),
            $section( [ $widget( 'nsfmeridiano_market_indicators', [ 'items' => [ 'oficial', 'tarjeta', 'blue', 'mep', 'ccl', 'riesgo' ], 'source_text' => 'Fuente: DolarAPI / ArgentinaDatos' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_middle_ad', [ 'placement' => 'middle', 'format' => 'leaderboard', 'label' => 'PUBLICIDAD', 'placeholder_text' => '728 × 90' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_latest_box', [ 'title' => 'Lo Último', 'section_label' => 'Argentina', 'posts_per_page' => 5 ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_news_row', [ 'posts_per_page' => 4, 'show_opinion_badge' => 'yes' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_middle_ad', [ 'placement' => 'middle', 'format' => 'billboard', 'label' => 'PUBLICIDAD', 'placeholder_text' => '970 × 250' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_feature_split', [ 'source' => 'manual', 'title' => 'La noticia dMeridiano que todos están comentando', 'text' => 'Un bloque destacado para campañas, especiales, videos o informes de alto impacto con estética editorial con cards y portada de alto impacto y foco visual.' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_footer_ad', [ 'placement' => 'footer', 'format' => 'footer-wide', 'label' => 'PUBLICIDAD', 'placeholder_text' => '970 × 120' ] ) ] ),
            $section( [ $widget( 'nsfmeridiano_footer_news', [] ) ] ),
        ];
    }

    private function demo_content( $title ) {
        return '<p>' . esc_html( $title ) . ' es una nota demo creada automáticamente para probar la experiencia popular de Meridiano.</p>'
