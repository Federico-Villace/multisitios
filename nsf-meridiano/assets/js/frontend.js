(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfmeridianoReady === '1') return;
    scope.dataset.nsfmeridianoReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfmeridiano-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfmeridiano-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfmeridiano-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfmeridiano-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfmeridiano-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfmeridiano-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfmeridiano-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfmeridiano-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfmeridiano-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfmeridiano-search-open');
      if (!document.body.classList.contains('nsfmeridiano-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfmeridiano-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfmeridiano-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfmeridiano-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfmeridiano_header_news.default', initAll);
  }
})();
