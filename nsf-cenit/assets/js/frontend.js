(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfcenitReady === '1') return;
    scope.dataset.nsfcenitReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfcenit-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfcenit-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfcenit-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfcenit-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfcenit-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfcenit-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfcenit-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfcenit-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfcenit-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfcenit-search-open');
      if (!document.body.classList.contains('nsfcenit-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfcenit-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfcenit-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfcenit-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfcenit_header_news.default', initAll);
  }
})();
