(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfarenaReady === '1') return;
    scope.dataset.nsfarenaReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfarena-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfarena-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfarena-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfarena-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfarena-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfarena-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfarena-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfarena-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfarena-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfarena-search-open');
      if (!document.body.classList.contains('nsfarena-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfarena-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfarena-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfarena-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfarena_header_news.default', initAll);
  }
})();
