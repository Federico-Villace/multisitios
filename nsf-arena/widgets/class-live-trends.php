...
class Live_Trends extends Widget_Base {
    public function get_name() { return 'nsfarena_live_trends'; }
    public function get_title() { return esc_html__( 'Live Bar + Trends', 'nsfarena-widgets' ); }
    public function get_icon() { return 'eicon-flash'; }

    protected function register_controls() {
        $this->start_controls_section( 'content_live', [ 'label' => esc_html__( 'Live Bar', 'nsfarena-widgets' ) ] );
        ...
        $this->add_control( 'live_title', [ 'label' => esc_html__( 'Título vivo', 'nsfarena-widgets' ), 'type' => \Elementor\Controls_Manager::TEXT, 'default' => 'Liga Profesional · Estudio Central', 'condition' => [ 'show_live_bar' => 'yes' ] ] );
        ...
        $this->start_controls_section( 'content_trends', [ 'label' => esc_html__( 'Trends', 'nsfarena-widgets' ) ] );
        ...
        $this->add_control( 'trends', [ 'label' => esc_html__( 'Trends manuales separados por coma', 'nsfarena-widgets' ), 'type' => \Elementor\Controls_Manager::TEXTAREA, 'default' => 'Mundial 2026,VIVO Liga Profesional,Inteligencia Artificial,Dólar,Vaca Muerta,Reforma previsional', 'condition' => [ 'trends_source' => 'manual' ] ] );
    ...
    private function get_google_trends_items( $geo, $limit, $cache_minutes ) {
        ...
        $cache_key = 'nsfarena_google_trends_' . strtolower( $geo ) . '_' . $limit;
        $cached = get_transient( $cache_key );
        if ( is_array( $cached ) ) { return $cached; }

        $url = add_query_arg( [ 'geo' => $geo, 'hl' => 'es-419' ], 'https://trends.google.com/trending/rss' );
        $response = wp_remote_get( $url, [ 'timeout' => 8, 'redirection' => 3, 'user-agent' => 'WordPress/nsfarena-widgets' ] );
        $items = [];
    ...
    protected function render() {
        ...
        ?>
        <div class="nsfarena-scope nsfarena-live-trends-widget">
            ...
            <div class="live-bar"><div class="container"><div class="live-bar-inner">
                ...
                <?php $live_content = '<span class="live-status"><span class="dot" aria-hidden="true"></span>' . esc_html( $s['live_label'] ?: 'EN VIVO' ) . '</span><span class="live-title">' . esc_html( $s['live_title'] ) . '</span><span class="live-time">' . esc_html( $s['live_time'] ?: '' ) . '</span><span class="live-chevron" aria-hidden="true">›</span>'; echo $live_url ? '<a class="nsfarena-live-main-link" href="' . esc_url( $live_url ) . '">' . $live_content . '</a>' : $live_content; ?>
            </div></div></div>
            <?php endif; ?>
