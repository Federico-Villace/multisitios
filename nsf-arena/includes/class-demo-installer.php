...
class Demo_Installer {
    private $option_key = 'nsfarena_demo_ids';
    ...
    public function install() {
        ...
        $pages = [
            ' Demo Completo' => '[nsfarena_demo view="cat" switcher="yes"]',
                        ' Nota'          => '[nsfarena_demo view="nota" switcher="no"]',
            ' Categoría'     => '[nsfarena_demo view="cat" switcher="no"]',
        ];
        ...
        foreach ( $pages as $title => $shortcode ) {
            ...
            if ( ! is_wp_error( $page_id ) ) {
                update_post_meta( $page_id, '_nsfarena_demo_page', 1 );
        ...
        if ( ! is_wp_error( $blocks_page_id ) ) {
            update_post_meta( $blocks_page_id, '_nsfarena_demo_page', 1 );
    ...
    private function elementor_blocks_data() {
        ...
        return [
            $section( [ $widget( 'nsfarena_header_news', [] ) ] ),
            $section( [ $widget( 'nsfarena_market_indicators', [ 'items' => [ 'oficial', 'tarjeta', 'blue', 'mep', 'ccl', 'riesgo' ], 'source_text' => 'Fuente: DolarAPI / ArgentinaDatos' ] ) ] ),
            $section( [ $widget( 'nsfarena_middle_ad', [ 'placement' => 'middle', 'format' => 'leaderboard', 'label' => 'PUBLICIDAD', 'placeholder_text' => '728 × 90' ] ) ] ),
            $section( [ $widget( 'nsfarena_latest_box', [ 'title' => 'Lo Último', 'section_label' => 'Argentina', 'posts_per_page' => 5 ] ) ] ),
            $section( [ $widget( 'nsfarena_news_row', [ 'posts_per_page' => 4, 'show_opinion_badge' => 'yes' ] ) ] ),
            $section( [ $widget( 'nsfarena_middle_ad', [ 'placement' => 'middle', 'format' => 'billboard', 'label' => 'PUBLICIDAD', 'placeholder_text' => '970 × 250' ] ) ] ),
            $section( [ $widget( 'nsfarena_feature_split', [ 'source' => 'manual', 'title' => 'La noticia d que todos están comentando', 'text' => 'Un bloque destacado para campañas, especiales, videos o informes de alto impacto con estética editorial con cards y portada de alto impacto y foco visual.' ] ) ] ),
            $section( [ $widget( 'nsfarena_footer_ad', [ 'placement' => 'footer', 'format' => 'footer-wide', 'label' => 'PUBLICIDAD', 'placeholder_text' => '970 × 120' ] ) ] ),
            $section( [ $widget( 'nsfarena_footer_news', [] ) ] ),
        ];
    }
