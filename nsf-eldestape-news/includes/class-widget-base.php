...
abstract class Widget_Base extends \Elementor\Widget_Base {
    public function get_categories() {
        return [ 'nsfeldestape-widgets' ];
    }

    public function get_style_depends() {
        return [ 'nsfeldestape-widgets-frontend' ];
    }
    ...
    protected function add_query_controls( $default = 4 ) {
        $this->add_control(
            'posts_per_page',
            [
                'label'   => esc_html__( 'Cantidad de notas', 'nsfeldestape-widgets' ),
        ...
        $this->add_control(
            'categories',
            [
                'label'       => esc_html__( 'Categorías', 'nsfeldestape-widgets' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => 'date',
                'options' => [
                    'date'     => esc_html__( 'Fecha', 'nsfeldestape-widgets' ),
                    'views'    => esc_html__( 'Vistas', 'nsfeldestape-widgets' ),
                    'comments' => esc_html__( 'Comentarios', 'nsfeldestape-widgets' ),
                    'rand'     => esc_html__( 'Aleatorio', 'nsfeldestape-widgets' ),
                ],
            ]
        );
    }
    ...
    protected function render_card( $post_id, $args = [] ) {
        ...
        if ( ! empty( $args['show_author'] ) ) {
            $byline = Helpers::author_byline( $post_id, $args['author_prefix'] ?? __( 'Por', 'nsfeldestape-widgets' ) );
