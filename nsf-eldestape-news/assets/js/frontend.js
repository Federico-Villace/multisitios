(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfeldestapeReady === '1') return;
    scope.dataset.nsfeldestapeReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfeldestape-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfeldestape-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfeldestape-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfeldestape-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfeldestape-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfeldestape-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfeldestape-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfeldestape-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfeldestape-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfeldestape-search-open');
      if (!document.body.classList.contains('nsfeldestape-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfeldestape-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfeldestape-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfeldestape-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfeldestape_header_news.default', initAll);
  }
})();
