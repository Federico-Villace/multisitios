...
class News_Row extends Widget_Base {
    public function get_name() { return 'nsfmarea_news_row'; }
    public function get_title() { return esc_html__( 'Fila de notas editorial', 'nsfmarea-widgets' ); }
            ...
            <?php $i=0; while ( $q->have_posts() ) : $q->the_post(); $i++; ?>
                <article class="nsfmarea-news-row-card">
                    ...
                    <?php if ( 'yes' === ( $s['show_author'] ?? 'yes' ) ) : ?>
                        <p class="nsfmarea-news-row-author">
                            <?php if ( 2 === $i && 'yes' === ( $s['show_opinion_badge'] ?? '' ) ) : ?><span class="nsfmarea-opinion-dot"></span><strong>OPINIÓN</strong><br><?php endif; ?>
                            <?php $byline = Helpers::author_byline( get_the_ID(), $s['author_prefix'] ?? __( 'Por', 'nsfmarea-widgets' ) ); if ( $byline ) { echo esc_html( $byline ); } ?>
                        </p>
                    <?php endif; ?>
                    <?php if ( 'yes' === ( $s['show_image'] ?? 'yes' ) ) : ?>
                        <a class="nsfmarea-news-row-img" href="<?php the_permalink(); ?>"><?php if ( has_post_thumbnail() ) { the_post_thumbnail( 'medium_large' ); } else { echo '<span class="card-img ' . esc_attr( \NSFMAREA\Helpers::card_image_class( get_the_ID() ) ) . '"></span>'; } ?></a>
                    <?php endif; ?>
                </article>
