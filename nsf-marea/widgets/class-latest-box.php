...
class Latest_Box extends Widget_Base {
    public function get_name() { return 'nsfmarea_latest_box'; }
    public function get_title() { return esc_html__( 'Lo Último Box', 'nsfmarea-widgets' ); }
        ...
        <div class="nsfmarea-scope"><div class="nsfmarea-latest-box">
            ...
            <div class="nsfmarea-latest-list">
                <?php $i = 0; while ( $q->have_posts() ) : $q->the_post(); $i++; ?>
                    <article class="nsfmarea-latest-item <?php echo 1 === $i && 'yes' === ( $s['first_with_thumb'] ?? 'yes' ) ? 'has-thumb' : ''; ?>">
                        <div class="nsfmarea-latest-text"><a class="nsfmarea-latest-item-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a></div>
                        <?php if ( 1 === $i && 'yes' === ( $s['first_with_thumb'] ?? 'yes' ) ) : ?>
                            <a class="nsfmarea-latest-thumb" href="<?php the_permalink(); ?>"><?php if ( has_post_thumbnail() ) { the_post_thumbnail( 'thumbnail' ); } else { echo '<span class="card-img sq ' . esc_attr( \NSFMAREA\Helpers::card_image_class( get_the_ID() ) ) . '"></span>'; } ?></a>
                        <?php endif; ?>
                    </article>
