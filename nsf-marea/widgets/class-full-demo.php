...
class Full_Demo extends Widget_Base {
    public function get_name() { return 'nsfmarea_full_demo'; }
    public function get_title() { return esc_html__( 'Maqueta completa HTML5', 'nsfmarea-widgets' ); }
    public function get_icon() { return 'eicon-site-identity'; }

    protected function register_controls() {
        $this->start_controls_section( 'section_content', [ 'label' => esc_html__( 'Maqueta', 'nsfmarea-widgets' ) ] );
        $this->add_control(
            'default_view',
            [
                'label'   => esc_html__( 'Vista inicial', 'nsfmarea-widgets' ),
                'type'    => \Elementor\Controls_Manager::SELECT,
                'default' => 'cat',
                'options' => [
                                        'nota' => esc_html__( 'Nota', 'nsfmarea-widgets' ),
                    'cat'  => esc_html__( 'Categoría', 'nsfmarea-widgets' ),
                ],
            ]
        );
        $this->add_control(
            'show_switcher',
            [
                'label'        => esc_html__( 'Mostrar switch demo', 'nsfmarea-widgets' ),
