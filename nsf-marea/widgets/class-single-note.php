...
class Single_Note extends Widget_Base {
    public function get_name() { return 'nsfmarea_single_note'; }
    public function get_title() { return esc_html__( 'Página de Nota', 'nsfmarea-widgets' ); }
    ...
    protected function register_controls() {
        ...
        $this->add_control( 'show_breadcrumb', [ 'label' => esc_html__( 'Mostrar breadcrumb/categoría', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
        $this->add_control( 'breadcrumb_home_text', [ 'label' => esc_html__( 'Texto inicio', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::TEXT, 'default' => esc_html__( 'Inicio', 'nsfmarea-widgets' ), 'condition' => [ 'show_breadcrumb' => 'yes' ] ] );
        ...
        $this->add_control( 'author_prefix', [ 'label' => esc_html__( 'Prefijo autor', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::TEXT, 'default' => esc_html__( 'Por', 'nsfmarea-widgets' ), 'description' => esc_html__( 'Dejalo vacío para mostrar sólo el nombre. Nunca imprime el prefijo si no hay autor.', 'nsfmarea-widgets' ), 'condition' => [ 'show_author' => 'yes' ] ] );
        ...
        $this->add_control( 'show_share', [ 'label' => esc_html__( 'Mostrar compartir', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
        $this->add_control( 'show_save', [ 'label' => esc_html__( 'Mostrar guardar', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes', 'condition' => [ 'show_share' => 'yes' ] ] );
        $this->end_controls_section();

        $this->start_controls_section( 'style_main', [ 'label' => esc_html__( 'Estilo', 'nsfmarea-widgets' ), 'tab' => \Elementor\Controls_Manager::TAB_STYLE ] );
        $this->add_responsive_control( 'content_width', [ 'label' => esc_html__( 'Ancho contenido', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::SLIDER, 'size_units' => [ 'px', '%' ], 'range' => [ 'px' => [ 'min' => 480, 'max' => 1200 ], '%' => [ 'min' => 40, 'max' => 100 ] ], 'selectors' => [ '{{WRAPPER}} .note-main' => 'max-width: {{SIZE}}{{UNIT}};' ] ] );
        $this->add_group_control( \Elementor\Group_Control_Typography::get_type(), [ 'name' => 'title_typo', 'label' => esc_html__( 'Título', 'nsfmarea-widgets' ), 'selector' => '{{WRAPPER}} .note-title' ] );
        $this->add_group_control( \Elementor\Group_Control_Typography::get_type(), [ 'name' => 'subtitle_typo', 'label' => esc_html__( 'Bajada', 'nsfmarea-widgets' ), 'selector' => '{{WRAPPER}} .note-subtitle' ] );
        $this->add_group_control( \Elementor\Group_Control_Typography::get_type(), [ 'name' => 'body_typo', 'label' => esc_html__( 'Cuerpo', 'nsfmarea-widgets' ), 'selector' => '{{WRAPPER}} .note-body' ] );
        $this->add_control( 'title_color', [ 'label' => esc_html__( 'Color título', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::COLOR, 'selectors' => [ '{{WRAPPER}} .note-title' => 'color: {{VALUE}};' ] ] );
        $this->add_control( 'accent_color', [ 'label' => esc_html__( 'Color acento', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::COLOR, 'default' => '#a82b1e', 'selectors' => [ '{{WRAPPER}} .note-breadcrumb a, {{WRAPPER}} .author-name strong, {{WRAPPER}} .note-body a' => 'color: {{VALUE}};', '{{WRAPPER}} .share-btn:hover' => 'border-color: {{VALUE}};' ] ] );
        $this->end_controls_section();

        $this->start_controls_section( 'style_share', [ 'label' => esc_html__( 'Compartir', 'nsfmarea-widgets' ), 'tab' => \Elementor\Controls_Manager::TAB_STYLE, 'condition' => [ 'show_share' => 'yes' ] ] );
        $this->add_responsive_control( 'share_size', [ 'label' => esc_html__( 'Tamaño botón', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::SLIDER, 'range' => [ 'px' => [ 'min' => 24, 'max' => 70 ] ], 'default' => [ 'size' => 38, 'unit' => 'px' ], 'selectors' => [ '{{WRAPPER}} .share-btn' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};' ] ] );
        $this->add_control( 'share_color', [ 'label' => esc_html__( 'Color iconos', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::COLOR, 'selectors' => [ '{{WRAPPER}} .share-btn, {{WRAPPER}} .share-save' => 'color: {{VALUE}};' ] ] );
        $this->add_control( 'share_hover_bg', [ 'label' => esc_html__( 'Fondo hover', 'nsfmarea-widgets' ), 'type' => \Elementor\Controls_Manager::COLOR, 'default' => '#1b1a17', 'selectors' => [ '{{WRAPPER}} .share-btn:hover' => 'background: {{VALUE}}; color: #fff; border-color: {{VALUE}};' ] ] );
        $this->end_controls_section();
    }
    ...
    protected function render() {
        ...
        $author_byline = Helpers::author_byline( $post_id, $s['author_prefix'] ?? __( 'Por', 'nsfmarea-widgets' ) );
        ...
        ?>
        <div class="nsfmarea-scope nsfmarea-np nsfmarea-note-widget"><div class="container"><article class="note-wrap"><div class="note-main">
            <?php if ( 'yes' === ( $s['show_breadcrumb'] ?? 'yes' ) ) : ?>
                <nav class="note-breadcrumb" aria-label="<?php echo esc_attr__( 'Migas', 'nsfmarea-widgets' ); ?>">
                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo esc_html( $s['breadcrumb_home_text'] ?: __( 'Inicio', 'nsfmarea-widgets' ) ); ?></a>
            ...
            <?php if ( 'yes' === ( $s['show_share'] ?? '' ) ) : ?>
                <div class="share-row">
                    ...
                    <?php if ( 'yes' === ( $s['show_save'] ?? 'yes' ) ) : ?><a href="#" class="share-save"><?php echo $this->icon_svg( 'bookmark' ); ?> <?php echo esc_html__( 'Guardar', 'nsfmarea-widgets' ); ?></a><?php endif; ?>
                </div>
            <?php endif; ?>
