...
abstract class Widget_Base extends \Elementor\Widget_Base {
    public function get_categories() {
        return [ 'nsfmarea-widgets' ];
    }

    public function get_style_depends() {
        return [ 'nsfmarea-widgets-frontend' ];
    }
    ...
    protected function add_query_controls( $default = 4 ) {
        $this->add_control(
            'posts_per_page',
            [
                'label'   => esc_html__( 'Cantidad de notas', 'nsfmarea-widgets' ),
        ...
        $this->add_control(
            'categories',
            [
                'label'       => esc_html__( 'Categorías', 'nsfmarea-widgets' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => 'date',
                'options' => [
                    'date'     => esc_html__( 'Fecha', 'nsfmarea-widgets' ),
                    'views'    => esc_html__( 'Vistas', 'nsfmarea-widgets' ),
                    'comments' => esc_html__( 'Comentarios', 'nsfmarea-widgets' ),
                    'rand'     => esc_html__( 'Aleatorio', 'nsfmarea-widgets' ),
                ],
            ]
        );
    }
    ...
    protected function render_card( $post_id, $args = [] ) {
        ...
        if ( ! empty( $args['show_author'] ) ) {
            $byline = Helpers::author_byline( $post_id, $args['author_prefix'] ?? __( 'Por', 'nsfmarea-widgets' ) );
