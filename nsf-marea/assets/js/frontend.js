(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfmareaReady === '1') return;
    scope.dataset.nsfmareaReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfmarea-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfmarea-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfmarea-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfmarea-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfmarea-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfmarea-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfmarea-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfmarea-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfmarea-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfmarea-search-open');
      if (!document.body.classList.contains('nsfmarea-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfmarea-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfmarea-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfmarea-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfmarea_header_news.default', initAll);
  }
})();
