(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfcapitalReady === '1') return;
    scope.dataset.nsfcapitalReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfcapital-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfcapital-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfcapital-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfcapital-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfcapital-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfcapital-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfcapital-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfcapital-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfcapital-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfcapital-search-open');
      if (!document.body.classList.contains('nsfcapital-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfcapital-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfcapital-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfcapital-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfcapital_header_news.default', initAll);
  }
})();
