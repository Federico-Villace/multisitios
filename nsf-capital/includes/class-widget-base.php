...
abstract class Widget_Base extends \Elementor\Widget_Base {
    public function get_categories() {
        return [ 'nsfcapital-widgets' ];
    }

    public function get_style_depends() {
        return [ 'nsfcapital-widgets-frontend' ];
    }
    ...
    protected function add_query_controls( $default = 4 ) {
        $this->add_control(
            'posts_per_page',
            [
                'label'   => esc_html__( 'Cantidad de notas', 'nsfcapital-widgets' ),
        ...
        $this->add_control(
            'categories',
            [
                'label'       => esc_html__( 'Categorías', 'nsfcapital-widgets' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => 'date',
                'options' => [
                    'date'     => esc_html__( 'Fecha', 'nsfcapital-widgets' ),
                    'views'    => esc_html__( 'Vistas', 'nsfcapital-widgets' ),
                    'comments' => esc_html__( 'Comentarios', 'nsfcapital-widgets' ),
                    'rand'     => esc_html__( 'Aleatorio', 'nsfcapital-widgets' ),
                ],
            ]
        );
    }
    ...
    protected function render_card( $post_id, $args = [] ) {
        ...
        if ( ! empty( $args['show_author'] ) ) {
            $byline = Helpers::author_byline( $post_id, $args['author_prefix'] ?? __( 'Por', 'nsfcapital-widgets' ) );
