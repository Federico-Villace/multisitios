(function () {
  ...
  function initAldia(scope) {
    if (!scope || scope.dataset.nsfatlasReady === '1') return;
    scope.dataset.nsfatlasReady = '1';

    var year = q(scope, '#footYear');
    if (year) year.textContent = new Date().getFullYear();

    var drawer = q(scope, '[data-nsfatlas-drawer]') || q(scope, '#drawer');
    var backdrop = q(scope, '[data-nsfatlas-backdrop]') || q(scope, '#backdrop, .drawer-backdrop');
    var burger = q(scope, '[data-nsfatlas-open]') || q(scope, '#burger');
    var drawerClose = q(scope, '[data-nsfatlas-close]') || q(scope, '#drawerClose');
    var searchOpen = q(scope, '[data-nsfatlas-search-open]');
    var searchMode = scope.getAttribute('data-search-mode') || 'ivory_modal';
    var searchModal = q(scope, '[data-nsfatlas-search-modal]');
    ...
    function openDrawer(focusSearch) {
      ...
      document.body.classList.add('nsfatlas-drawer-open');
    ...
    function closeDrawer() {
      ...
      document.body.classList.remove('nsfatlas-drawer-open');
    ...
    function openSearchModal() {
      ...
      document.body.classList.add('nsfatlas-search-open');
      document.body.style.overflow = 'hidden';
      window.setTimeout(function () {
    ...
    function closeSearchModal() {
      ...
      document.body.classList.remove('nsfatlas-search-open');
      if (!document.body.classList.contains('nsfatlas-drawer-open')) document.body.style.overflow = '';
    }
    ...
    qa(scope, '[data-nsfatlas-search-close]').forEach(function (el) {
    ...
    var initialView = scope.getAttribute('data-nsfatlas-default-view') || 'home';
  ...
  function initAll() {
    document.querySelectorAll('.nsfatlas-scope').forEach(initAldia);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    ...
    window.elementorFrontend.hooks.addAction('frontend/element_ready/nsfatlas_header_news.default', initAll);
  }
})();
